...
public func menuBar(viewModel: TrayMenuModel) -> some Scene {
  MenuBarExtra {
    ...
    if viewModel.isEnabled {
      WindowIconList(workspaceDisplayInfo: Workspace.get(byName: viewModel.trayText).displayInfo)
    } else {
      Text("?")
    }
  }
    ...
    let renderer = ImageRenderer(
      content:
        HStack {
          ...
          ForEach(Array(bundleIdentifiers.enumerated()), id: \.offset) { index, bundleIdentifier in
            WindowIndicator(
