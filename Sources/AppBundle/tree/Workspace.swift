...
private func getStubWorkspace(forPoint point: CGPoint) -> Workspace {
    ...
    return (1...Int.max).lazy
        ...
        .first {
            $0.isEffectivelyEmpty && !$0.isVisible && !preservedNames.contains($0.name)
                && $0.forceAssignedMonitor == nil
        }
        ?? errorT("Can't create empty workspace")
}

struct WorkspaceDisplayInfo {
    let name: String
    let bundleIdentifiers: [String]
    let indexOfFocussed: Int?
}
...
class Workspace: TreeNode, NonLeafTreeNodeObject, Hashable, CustomStringConvertible, Comparable {
    ...
    var displayInfo: WorkspaceDisplayInfo {
        ...
        if let focussedWindow = focus.windowOrNil,
            let index = windowsWithBundleIdentifiers.firstIndex(of: focussedWindow)
        {
            indexOfFocussed = index
        }

        return WorkspaceDisplayInfo(
            name: name,
            bundleIdentifiers: bundleIdentifiers,
            indexOfFocussed: indexOfFocussed
        )
    }

    var windowsDisplayString: String {
        let windows = self.allLeafWindowsRecursive

        var windowShorts = windows.map {
            let name = $0.asMacWindow().macApp.name ?? ""
            return String(name.replacingOccurrences(of: "\u{200E}", with: "").prefix(2))
        }

        if let focussedWindow = focus.windowOrNil,
            let indexOfFocussed = windows.firstIndex(of: focussedWindow)
        {
            windowShorts[indexOfFocussed] = "(" + windowShorts[indexOfFocussed] + ")"
        }

        return windowShorts.joined(separator: " ")
    }
    ...
    static func garbageCollectUnusedWorkspaces() {
        ...
        workspaceNameToWorkspace = workspaceNameToWorkspace.filter { (_, workspace: Workspace) in
            preservedNames.contains(workspace.name) || !workspace.isEffectivelyEmpty
                || workspace.isVisible || workspace.name == focus.workspace.name
        }
    }
...
private func rearrangeWorkspacesOnMonitors() {
    ...
    for newScreen in newScreens {
        if let existingVisibleWorkspace = newScreenToOldScreenMapping[newScreen]?.lets({
            oldScreenPointToVisibleWorkspace[$0]
        }),
        ...
        check(
            newScreen.setActiveWorkspace(stubWorkspace),
...
private func isValidAssignment(workspace: Workspace, screen: CGPoint) -> Bool {
    if let forceAssigned = workspace.forceAssignedMonitor,
        forceAssigned.rect.topLeftCorner != screen
    {
        return false
    }
    else {
